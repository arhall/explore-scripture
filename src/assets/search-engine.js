...
class SearchEngine {
      ...
      if (book.chapterSummaries) {
        Object.entries(book.chapterSummaries).forEach(([chapterNum, summary]) => {
          bookItems.push({
            ...
            searchText: this.buildSearchText([book.name, `chapter ${chapterNum}`, summary]),
            relevanceBoost: 1.0,
            bookName: book.name,
  ...
  buildEntitiesIndex() {
    ...
    entities.forEach(entity => {
      entityItems.push({
        ...
        description:
          entity.blurb ||
          `A ${entity.type} mentioned in ${entity.books.length} book${entity.books.length !== 1 ? 's' : ''}: ${entity.books.slice(0, 3).join(', ')}${entity.books.length > 3 ? '...' : ''}`,
        url: entity.url,
  ...
  getEntityRelevanceBoost(entityType, referenceCount) {
    ...
    const referenceBoost =
      referenceCount > 0 ? Math.min(Math.log10(referenceCount + 1) * 0.5, 2.0) : 0;
