...
class DataOptimizer {
  ...
  async createBinaryIndexes() {
    ...
    try {
      ...
      for (const [indexName, indexData] of Object.entries(indexes)) {
        ...
        console.log(
          `   Created index: ${indexName} (${this.formatBytes(fs.statSync(binaryPath).size)} binary)`
        );
      }
  ...
  createFullTextIndex(entities) {
    ...
    entities.forEach(entity => {
      const text = [entity.name, entity.type, entity.role, entity.searchText]
        .filter(Boolean)
        .join(' ')
        .toLowerCase();
    ...
    return {
      ...
      stats: {
        ...
        averageEntitiesPerWord:
          Object.values(index).reduce((sum, entities) => sum + entities.length, 0) /
  ...
  async createCompressedChunks() {
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        console.log(
          `   Chunk ${i} (${chunk.priority}): ${this.formatBytes(jsonData.length)} → ${this.formatBytes(compressedData.length)} (${savings}% savings)`
        );
      }
  ...
  async optimize() {
    ...
    const results = {
      ...
      bloomFilter: this.createBloomFilter(
        JSON.parse(fs.readFileSync(path.join(this.dataDir, 'entities-search.json'), 'utf8'))
      ),
    };
