...
async function generateMissingEntityFiles() {
  try {
    ...
    for (const bookSlug of missingBooks) {
      ...
      const bookEntities = entitiesData.filter(entity => 
        entity.books && entity.books.includes(bookName)
      );
      ...
      if (bookEntities.length > 0) {
        ...
        const outputPath = path.join(__dirname, '../src/assets/data/books', `${bookSlug}-entities.json`);
        ...
        if (bookEntities.length > 0) {
          const sampleNames = bookEntities.slice(0, 3).map(e => e.name).join(', ');
          console.log(`   📝 Sample entities: ${sampleNames}${bookEntities.length > 3 ? '...' : ''}`);
        }
      } else {
    ...
    console.log('\n🔍 Verification:');
    for (const bookSlug of missingBooks) {
      const filePath = path.join(__dirname, '../src/assets/data/books', `${bookSlug}-entities.json`);
